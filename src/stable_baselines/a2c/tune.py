"""
Optuna hyperparameter search for A2C.
Run from project root:
    python src/stable_baselines/a2c/tune.py

Results are saved to src/models/optuna/a2c_size{SIZE}.db (SQLite).
After the study finishes, best params are printed and written to
src/models/optuna/a2c_size{SIZE}_best_params.json.
"""
...
def main():
    ...
    db_path = os.path.join(OPTUNA_DIR, f'a2c_size{SIZE}.db')
    out_path = os.path.join(OPTUNA_DIR, f'a2c_size{SIZE}_best_params.json')

    study = optuna.create_study(
        study_name=f'a2c_size{SIZE}',
        direction='maximize',
